...
const customFieldService = {
  ...
  create: async (fieldData: Partial<CustomField>): Promise<CustomField> => {
    try {
      const response = await axios.post(`${API_URL}/custom-fields`, fieldData, {
        headers: {
          Authorization: `Bearer ${localStorage.getItem('token') || 'mock-token-for-development'}`
        }
      });
  ...
  update: async (id: string, fieldData: Partial<CustomField>): Promise<CustomField> => {
    try {
      const response = await axios.put(`${API_URL}/custom-fields/${id}`, fieldData, {
        headers: {
          Authorization: `Bearer ${localStorage.getItem('token') || 'mock-token-for-development'}`
        }
      });
